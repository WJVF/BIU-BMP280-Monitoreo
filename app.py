...
def cargar_datos():
    ...
    data = ref.order_by_key().limit_to_last(46500).get()
    if not data:
        return pd.DataFrame()
...
Usa esta fecha como referencia para interpretar expresiones relativas como
"esta semana", "semana pasada", "ayer", "el mes pasado", etc. Calcula el rango
de fechas correspondiente y compáralo contra el RESUMEN POR DÍA proporcionado
abajo antes de concluir que no hay datos disponibles.
